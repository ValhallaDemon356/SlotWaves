    ...
    public function validate(string $selectedReportType, $file, bool $isProbe = false, ?string $originalFilename = null): array
    {
        ...
        if (strcasecmp($selectedReportType, 'fdr') === 0) {
            ...
            return [
                ...
                'detected_format'  => $fdrResult['detected_format'] ?? 'OASYS HTML XLS',
                'file_name'        => $originalFilename ?: basename($filePath),
                'airport'          => $fdrResult['airport'] ?? 'CGK',
                'operator'         => $fdrResult['operator'] ?? 'ALL AIRLINE',
                'period_label'     => $fdrResult['period_label'] ?? '',
                'date_start'       => $fdrResult['date_start'] ?? '',
                'date_end'         => $fdrResult['date_end'] ?? '',
                'direction'        => $fdrResult['direction'] ?? 'ALL',
                'leg'              => $fdrResult['leg'] ?? ($fdrResult['direction'] ?? 'ALL'),
                'route_type'       => $fdrResult['route_type'] ?? 'ALL',
                'realization'      => $fdrResult['realization'] ?? 'YES',
                'detected_columns' => $conf['detected_columns'],
